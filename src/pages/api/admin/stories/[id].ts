...
export const POST:APIRoute=async({request,cookies,locals,params,redirect})=>{
  ...
  const baseStory=await db.prepare("SELECT slug FROM stories WHERE id=? LIMIT 1").bind(id).first<{slug:string}>();
  const translationSlug=baseStory?.slug||slug;
  ...
  if(language!=="en"){
    await db.prepare(`INSERT INTO story_translations
      ...
      .bind(id,language,translationSlug,title,excerpt,content,tags,xp,status).run();
